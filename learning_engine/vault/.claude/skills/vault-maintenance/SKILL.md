...
description: Day-to-day operation of a learning-engine vault — today's study plan, review grading, mastery-tag sync (all 4 places), FSRS/FIRe upkeep, catch-up planning, index maintenance, and known failure modes. Use when the user asks what to study today, reports a review result, needs session processing, sees wrong mastery tags, or says the SRS seems off.
---
...
Run commands with `Set-Location vault; python ...`, and prefix Workspace file
paths with `vault/`.
