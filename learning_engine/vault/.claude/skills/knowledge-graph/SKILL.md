...
description: Add learnable knowledge or build a Math Academy-style prerequisite knowledge graph and Obsidian study vault for any subject — extract curricula into granular skill notes, mine prerequisite edges, validate the DAG, and wire mastery, FSRS, and FIRe. Use when the user wants to add a concept or skill to the learning system, create/extend a subject graph, re-mine prerequisites, or audit graph quality.
---
...
With the integrated Action gateway, `WORKSPACE_ROOT` is `learning_engine/`.
Workspace file paths therefore start with `vault/`, handbook paths start with
`docs/`, and commands should begin with `Set-Location vault`. Reuse the migrated
engine and pipeline; do not recreate them in another directory.
